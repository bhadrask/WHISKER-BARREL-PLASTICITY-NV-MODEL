function [lissom] = lat_dynamics_final_vas_intr(lissom,ii,opts,Z_constraint,ATP,Amax)
%number of time steps for setteling is predefined. Instead can be used threshold  
lissom_aff = (lissom.layers{ii}.Z).*(Z_constraint);
...
threshold= fnctn(thr,ATP);
        lissom.layers{ii}.Z  = lissom_aff +(opts.q*positive)-(opts.r*negative)-threshold;
