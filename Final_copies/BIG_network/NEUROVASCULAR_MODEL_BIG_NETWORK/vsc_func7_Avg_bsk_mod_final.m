function [dCMRO2ndt,OEn]=vsc_func7_Avg_bsk_mod_final(OE1,bndry,neu,CMRO2nref,W,filename,CMRO2n,PO2tis,PO2tismax)
...
for cj=1:size(neu,1)
    for ci=1:size(neu,2)
        ...
        dCMRO2ndt(count)=(0.7*OEn(count)*(neu(ci,cj))+(CMRO2nref(count)*factr(count)-CMRO2n(count)))/2;%c3 t3
    end
end
end