...
clc;
clear all;
inps= [ 2  5 15 25 40 80 200];% 25 40 80 200]; %500 denotes 0.5
for num=1:numel(inps)
    filename = sprintf('final_amp%d.mat',round(inps(num),1));
    load(filename);
% filename = 'final_amp200.mat';
load(filename);
k=size(x,1);
load('XY.mat');
...
%  disp(['j is ',num2str(j),' peak ',num2str(max(max(delHbO)))]);
%         if g==1600
           figure(); imagesc(delHbO);title(['Stimulus amplitude ',num2str(inps(num))]); colorbar;
h=colorbar;caxis([-0.15 0.2]);
           %         end
        g=g+200;
