...
for avg=1:loop
for num=1:1
filename = 'output_4_plot';%sprintf('r%d.mat',inps(num)) ;
...
if (g<1) || (g>2)
 ...
 Fpoint=caps2D(Fin1(end,find(lev==4)),4,0);
 Vpoint=caps2D(V1(end,find(lev==4)),4,0);
  Fpoint0=caps2D(Fin1(1,find(lev==4)),4,0);
...
figure();plot(time, Flow,'r');hold on;
plot( time,Vol,'b');hold on;
