function [lissom] = lat_dynamics_no_sigmoid(lissom,ii,opts,ATP,ATPmax,thr)
...
%  hitemp=0.8*max(max(lissom.layers{ii}.Z));
%  lotemp=0.2*max(max(lissom.layers{ii}.Z));
  lissom.layers{ii}.Z_nosig  =lissom.layers{ii}.Z ;
