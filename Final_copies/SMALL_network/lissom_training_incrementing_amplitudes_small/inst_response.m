function [yy]=inst_response(lissom,opts,X1,label,ATP,ATPmax)
yy=[];xx=[];
...
for i=1:size(X1,3)
    ...
    for jj=2:numel(lissom.layers)
        ...
        [lissom]=lat_dynamics_final_4_response(lissom,jj,opts,ATP,ATPmax);
    ...
    figure(2);
    subplot(2,1,2);imagesc(yy);colorbar;caxis([0 1]);title(['input',num2str(i),' max=',num2str(max(max(yy)))]); pause(0.5)
