...
threshold= fnctn(thr,ATP);
        ...
        lissom.layers{ii}.Zold  = lissom_aff +opts.q*(positive)-opts.r*(negative)-threshold;
...
disp(['maximum value before threshold = ',num2str(max(max(lissom.layers{ii}.Zold)))]);
 lissom.layers{ii}.Zoldpassed  = gactive(lissom.layers{ii}.Zold ,lo,hi);
disp(['maximum neural value = ',num2str(max(max(lissom.layers{ii}.Zoldpassed)))]);
