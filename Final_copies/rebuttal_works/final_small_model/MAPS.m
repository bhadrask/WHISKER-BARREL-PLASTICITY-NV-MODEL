function [lissom,label]= MAPS(lissom,opts,X1,label,ATP,ATPmax,thr) 
xx=[];
label(1,9)=-1;
opts.niter=1;
for i=1:size(X1,3)     %testing for 8 input videos
    ...
    for j=1:steps
        for jj=2:numel(lissom.layers)
            ...
            [lissom] = lat_dynamics_final_4_response(lissom,jj,opts,lissom.layers{jj}.padded_input,ATP,ATPmax,thr); %lateral dynamics
            if jj==2
