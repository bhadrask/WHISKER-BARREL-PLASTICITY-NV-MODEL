...
function [Pna,Pe]=vsc_func3_new(E,N1,N2,bndry,R,Pe,dVdt,para,lev,layers)
% e2=bndry(4);
n1=bndry(1);
n2=bndry(2);
% eA=bndry(5);
cap_layer=(numel(layers)+1)/2;
ind_precap=find(lev==cap_layer-1);
ind_start=N1(min(ind_precap));
% nE=length(E);
% nN=N2(end)+1;
Pa=para(1);
A=zeros(n1-1);
